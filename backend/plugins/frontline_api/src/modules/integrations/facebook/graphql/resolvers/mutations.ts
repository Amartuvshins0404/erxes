...
export const facebookMutations = {
  ...
  async facebookCreatePost(
    ...
    // Facebook keeps the photos and silently drops the link when both are
    // present, so the post would publish minus the link with no error.
    ...
    try {
      ...
      // Files the user uploaded to our own storage: we send Facebook the bytes
      // instead of a URL, so this works even where Facebook cannot reach us.
