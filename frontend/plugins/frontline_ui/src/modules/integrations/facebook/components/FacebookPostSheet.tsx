...
// core-api stores whatever filename you upload, but sanitizeKey refuses to read
// back anything outside this set -- so a name with a comma uploads fine and then
// 400s forever. Catch it here rather than let the post fail on a stored file
...
// The intersection of what erxes accepts on upload (defaultMimeTypes in
// core-api/utils/file) and what Facebook accepts as a photo. Offering more only
// moves the rejection later: AVIF and HEIC fail the erxes upload, and WebP
// would pass Facebook but not erxes.
...
export const FacebookPostSheet = () => {
  ...
  // Radix unmounts Sheet.View children when the sheet closes, so anything the
  // form owns is destroyed on close. The draft lives here instead, where it
  // survives until the post actually publishes.
...
const FacebookPostSheetForm = ({
  ...
  // The upload hook only drops SUCCESSFUL files from its list, so a rejected
  // file (wrong type, too large) stays there forever. Counting those as
  // "pending" would disable Publish permanently and strand the user.
  // Rejections arrive by two different routes: the server sets upload.errors,
  // while a client-side reject (wrong type, too large) is only recorded as
  // .errors ON the file inside upload.files and never reaches upload.errors.
  // Reading one list alone leaves the other kind invisible AND uncounted, so
  // they pile up until maxFiles is reached and every further drop is refused.
  const rejected = [
    ...upload.errors.map((e) => ({
      name: e.name,
      message: e.message || '',
  ...
  // Removing an image here only takes it out of this post. The uploaded file is
  // kept -- retaining what was uploaded is a deliberate product decision.
  const discardImage = (index: number) => {
    setImages((prev) => prev.filter((_, i) => i !== index));
  };

  // Upload as soon as files are dropped. Staging them behind a separate button
  // meant a user could drop images, hit Publish, and get a text-only post with
  // a success toast -- the images silently dropped.
  useEffect(() => {
    ...
    // Keyed on length, not files[0]: a stuck failed file keeps index 0 fixed,
    // so a newly dropped image would never trigger an upload.
          ...
          {postIntegrations.length === 0 ? (
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                {/* useErxesUpload hardcodes noClick, so the zone is
                    drag-only unless open() is wired up explicitly. */}
                <div
                  {...upload.getRootProps()}
                  onClick={upload.open}
                  role="button"
