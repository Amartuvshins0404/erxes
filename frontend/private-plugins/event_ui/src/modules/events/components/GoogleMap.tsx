...
import { REACT_APP_GOOGLE_MAP_API_KEY, themeState, useToast } from 'erxes-ui';
import { useAtomValue } from 'jotai';
import { useEffect, useState } from 'react';
import { MapConfig } from '@/events/types';
import { GoogleMapControl } from '@/events/components/GoogleMapControl';
...
export const GoogleMap = ({
  ...
  onSelect?: (value: IGoogleMapLocation) => void;
} = {}) => {
...
const MapContent = ({
  ...
  const handleClick = async (event: MapMouseEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Could not look up the address',
        description:
          'The pin was placed, but city/district/address must be entered manually.',
        variant: 'destructive',
      });
    }
  };
