...
import { BA_MEMBERSHIPS, BA_MEMBERSHIP_PLANS } from '../graphql/queries';
import { IPaymentMethod, IMember, IMembershipPlan } from '../types';
import { addMonths } from '../utils/grantHelpers';
import { useGrantMembership } from './useMemberActions';
...
export const useGrantSheetState = (open: boolean) => {
  ...
  const { data: subData } = useQuery(BA_MEMBERSHIPS, {
  ...
  const plans: IMembershipPlan[] = plansData?.baMembershipPlans?.list || [];
  ...
  const activeSub: IMember | undefined = (
    subData?.baMemberships?.list as IMember[] | undefined
  )?.find(
    (s) =>
