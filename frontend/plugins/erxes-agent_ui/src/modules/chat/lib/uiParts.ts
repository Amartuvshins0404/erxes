...
/** Whether a tool can produce an inline artifact card. */
export const isArtifactTool = (toolName: string): boolean =>
  ARTIFACT_TOOL_NAMES.has(toolName.toLowerCase().replace(/[-_\s]/g, ''));
...
export const pendingApproval = (
  ...
  return {
    ...
    prompt:
      summary || lastSentences(messageText(last)) || 'Confirm this action?',
    operations,
  };
};
