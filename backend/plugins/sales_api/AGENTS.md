...
### `2026-09-02` — Revert bounded agent-facing deal reads

- **Summary:** Cherry-picked upstream `247b231bbd` (PR #9210), reverting
  #9102: `deal.find` and `deal.count` return to loose `z.any()` inputs with
  MongoDB-style filters. The strict-shape contract broke agent-driven
  `deal.findOne`-dependent flows because it changed the calling convention
  the rest of the system relied on.
- **Affected areas:** `src/modules/sales/trpc/deal.ts`.
- **Contracts changed:** `deal.find` input is again a bare filter or
  `{ query, skip, limit, sort }` object with no bounding; `deal.count` again
  takes a bare filter object.
