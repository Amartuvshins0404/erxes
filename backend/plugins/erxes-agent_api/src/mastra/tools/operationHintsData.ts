...
export const OPERATION_HINTS: Record<string, OperationHint> = {
  ...
  salesBoards: {
    ...
    rules: [
      'for an explicit pipeline/deal coverage audit, request pipelines._id, pipelines.name, and pipelines.itemsTotalCount to get each board pipeline and its deal count in one read',
    ],
  },
  salesBoardCounts: {
    rules: [
  ...
  salesPipelines: {
    defaultResponseFields: ['_id', 'name', 'boardId'],
    rules: [
      'request itemsTotalCount only when deal counts are required; it is the deal count for each pipeline',
    ],
  },
  salesStages: {
    defaultResponseFields: ['_id', 'name', 'pipelineId'],
    rules: [
      'request itemsTotalCount only when deal counts are required; it is the deal count for each stage',
