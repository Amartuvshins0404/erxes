...
| Agency members | `backend/plugins/blockadmin_api/src/modules/member/` | Mirrored agent schema/model (`block_admin_members`), member webhook routes (`routes/member.ts`), admin GraphQL API, and `utils.ts`'s `resolveAgencyKeys`/`resolveAgentKeys`/`findAgentAgency` id translation |
