...
import { BA_SUPPLIERS } from '../graphql/queries';
...
export const useSuppliers = (options?: QueryHookOptions) => {
  ...
  const { data, loading, fetchMore } = useQuery<{
    baSuppliers: ISupplierList;
  }>(BA_SUPPLIERS, {
    ...options,
    variables: { ...variables, cursor, limit: SUPPLIERS_PER_PAGE },
  });

  const { list: suppliers, pageInfo, totalCount } = data?.baSuppliers || {};
    ...
    fetchMore({
      ...
      updateQuery: (prev, { fetchMoreResult }) => {
        ...
        return Object.assign({}, prev, {
          baSuppliers: mergeCursorData({
            direction,
            fetchMoreResult: fetchMoreResult.baSuppliers,
            prevResult: prev.baSuppliers,
          }),
        });
      },
    });
