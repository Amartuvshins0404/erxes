...
export const productColumns: ColumnDef<IBaProduct>[] = [
  RecordTable.checkboxColumn as ColumnDef<IBaProduct>,
  {
    id: 'name',
    accessorKey: 'name',
    header: () => {
    ...
    cell: ({ cell, row }) => {
      const [, setActiveProductId] = useQueryState<string>('activeProductId');
      return (
        <RecordTableInlineCell
  ...
  },
  {
    ...
    header: () => {
      const { t } = useTranslation('blockadmin');
      return (
        <RecordTable.InlineHead label={t('Category')} icon={IconCategory} />
      );
    },
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => {
      const { t } = useTranslation('blockadmin');
      return (
        <RecordTable.InlineHead label={t('Status')} icon={IconShieldCheck} />
      );
    },
    cell: ({ cell, row }) => {
      const { t } = useTranslation('blockadmin');
  ...
  },
  {
    ...
    header: () => {
      const { t } = useTranslation('blockadmin');
      return (
        <RecordTable.InlineHead label={t('Created')} icon={IconCalendar} />
      );
    },
    cell: ({ cell }) => (
