...
const promiseSchema = new Schema<IProfilePromise>(
  {
    title: { type: String, label: 'Promise title' },
...
const billSchema = new Schema<IProfileBill>(
  {
    title: { type: String, label: 'Bill title' },
    stage: {
      type: String,
...
const educationSchema = new Schema<IProfileEducation>(
  {
    school: { type: String, label: 'School' },
...
profileSchema.index({ subdomain: 1, entityId: 1 }, { unique: true });
profileSchema.index({ reviewStatus: 1, syncedAt: -1 });
