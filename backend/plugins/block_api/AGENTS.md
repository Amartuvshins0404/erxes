...
- Org-wide online-payment configuration (`blockGetContractPaymentSettings` / `blockUpdateContractPaymentSettings`): which payment_api payment-method ids may be charged, and whether a customer may pay less than a scheduled payment's remaining amount.
...
- GraphQL online payment: `blockCreateContractPaymentInvoice(paymentId, amount)` → `{invoiceId, url, amount, currency}`, `blockGetContractPaymentSettings`, `blockUpdateContractPaymentSettings(input)`.
...
- `block_contract_payment_settings` is an org-wide singleton (`paymentIds`, `allowPartial`); `ContractPaymentSettings.getSettings()` returns the first (and only) document, and `updateSettings` creates it on first write.
