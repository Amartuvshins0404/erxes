...
exercise erxes-agent's real **agents, tools, memory and processors** in
Studio's UI — reusing the production runtime, with no schema translation.
...
- The **gateway / erxes API on `:4000`** must be up — full-reuse agents build
  their tools from its operation registry.
- Enable memory from the tenant's Agent General Settings page for the memory
  and history tabs.
...
- **Memory / history** — native `getMastraMemory` (Mongo `erxes_mastra_memory`).
  `/api/memory/status` is `true` when tenant memory is enabled.
...
| `index.ts`           | builds `new Mastra({ agents, storage })`                      |
...
- **Build**: the bridge is excluded from `tsconfig.build.json` (`src/studio/**` +
  ...
  `mastra` CLI (esbuild). Your editor may show one cosmetic _top-level-await_
