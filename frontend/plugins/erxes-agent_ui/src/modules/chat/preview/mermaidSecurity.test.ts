...
describe('MermaidViewer security configuration (finding E)', () => {
  const source = readFileSync(join(__dirname, 'MermaidViewer.tsx'), 'utf8');
  ...
  it("never uses the unsafe 'loose' security level", () => {
    expect(source).not.toMatch(/securityLevel:\s*['"]loose['"]/);
  });
  ...
  it('passes the rendered SVG straight to PanZoomSvg (the injection sink)', () => {
    ...
    expect(source).toMatch(/<PanZoomSvg\s+svgHtml=\{svgHtml\}/);
  });
});
