...
/**
 * Attachments read through apollo carry `__typename`, which `AttachmentInput`
 * rejects. Keep only the input fields before sending them back to the api.
 */
export const toAttachmentInput = (
  attachment?: AgencyAttachment | null,
): AgencyAttachment | null => {
  if (!attachment) {
    return null;
  }

  const { url, name, type, size, duration } = attachment;

  return { url, name, type, size, duration };
};

export const toAttachmentInputs = (
  attachments?: (AgencyAttachment | null)[] | null,
): AgencyAttachment[] =>
  (attachments || [])
    .map(toAttachmentInput)
    .filter((attachment): attachment is AgencyAttachment => !!attachment);
